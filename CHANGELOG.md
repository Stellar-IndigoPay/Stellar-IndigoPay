...
- **backend:** snapshot the USDC→XLM rate (and its source) on each donation at ingestion, and use the stored rate for historical CO₂ attribution so later rate corrections or backfills no longer re-value past donations (closes #683)
