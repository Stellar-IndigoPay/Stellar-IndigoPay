...
### Fixed
- **contracts:** validate escrow campaign milestone vectors before storage so empty, zero-percentage, duplicate-name, oversized-name, and non-100% milestone sets are rejected at campaign creation, matching the escrow contract’s invariant checks.
- **backend:** Serialize `indexerBackfill` and `indexerReconciler` with a shared Postgres session-level advisory lock (`pg_try_advisory_lock`) so overlapping backfill/reconcile runs — whether from two replicas or a manual admin trigger racing the periodic cron — are serialised: the second caller returns `{ skipped: true }` immediately without duplicating any data (closes #119). Idempotent upserts on `donation_events` (unique partial indexes added in migration `030_indexer_backfill_lock`) provide a second, database-level guard so a duplicate insert is rejected by the constraint even if the lock is somehow bypassed. Adds a unit-level concurrency test in `indexerConcurrency.test.js`.
