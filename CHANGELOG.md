...
- **contracts/oracle:** add per-source staleness checking in `get_aggregated_price` by querying source timestamp/ledger (`last_updated_ledger`/`get_last_updated_ledger`) or tracking registered source ledger records against `staleness_threshold` to exclude stale source prices from median calculation (closes #709)
