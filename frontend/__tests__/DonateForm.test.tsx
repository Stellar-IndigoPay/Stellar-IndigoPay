...
describe("DonateForm — Workstream 1 (overview)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockIsOnline = true;
    mockGetAccountSummary.mockResolvedValue({ balance: "100", subentries: 0 });
  ...
  it("disables submission and shows an error when the amount exceeds the balance", async () => {
    mockGetAccountSummary.mockResolvedValue({ balance: "95", subentries: 0 });
    renderForm();
...
describe("DonateForm — Workstream 2 (offline queued confirmation)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockIsOnline = false;
    mockGetAccountSummary.mockResolvedValue({ balance: "500", subentries: 0 });
    mockGetBaseReserveXLM.mockResolvedValue(2);
    mockGetAssetBalance.mockResolvedValue(null);
  });

  it("shows an explicit queued-confirmation card instead of a dead form", async () => {
    renderForm();
    ...
    await waitFor(() => {
      expect(screen.getByTestId("donation-queued")).toBeInTheDocument();
    });
    expect(screen.getByTestId("donation-queued")).toHaveTextContent(
      /queued/i,
    );
  });
});
