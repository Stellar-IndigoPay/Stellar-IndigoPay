...
/**
 * Load a donor account once and return the XLM balance plus its subentry
 * count, so callers can compute the account's true minimum reserve
 * ((2 + subentries) × base reserve) — the reserve a Max-button donation
 * must leave intact.
 *
 * @param publicKey - Stellar account public key.
 * @returns Balance (XLM decimal string) and number of subentries.
...
export async function getAccountSummary(publicKey: string): Promise<{
  ...
  try {
    const account = await server.loadAccount(publicKey);
    const xlm = account.balances.find((b) => b.asset_type === "native");
    const subentries = (account as AccountResponseWithSubentries).num_subentries ?? 0;
    return { balance: xlm ? xlm.balance : "0", subentries };
  } catch {
    throw new Error("Account not found or not funded.");
  }
}
