 ...
 *  2. Offline durability — an offline donation is queued with an explicit
 *     confirmation card, and on reconnect it is recorded exactly once
 *     (server idempotency pre-check) with the queue badge reflecting the
 *     pending count.
 *
...
test.describe("Donation flow — V2 (preview + offline durability)", () => {
  ...
  test("queues an offline donation and records it exactly once on reconnect", async ({
    ...
    // Go offline and donate — the donation is queued with an explicit card.
    await context.setOffline(true);
    await click(page.getByTestId("donate-button"), testInfo);
    await expect(page.getByTestId("donation-queued")).toBeVisible();
    await expect(page.getByTestId("donation-queued")).toContainText(/queued/i);
  ...
  test("two tabs sharing the queue record the offline donation exactly once on reconnect", async ({
    ...
    await expect(tabB.getByTestId("wallet-connect-button").last()).toBeVisible();
    ...
    // Reconnect — BOTH tabs' drain routines fire. The BroadcastChannel tab
    // lock defers one of them, and the server idempotency contract dedupes
    // even in a submission race: exactly one donation record, both tabs'
    // badges clear.
    await context.setOffline(false);
    await expect
      .poll(() => backend.donations.length, { timeout: 15000 })
