...
impl AttestationContract {
    ...
    pub fn record_attestation(
        ...
        // Update on-chain aggregates for O(1) frontend queries.
        update_aggregates_on_record(&env, &donor, &source_chain, amount_usd, amount_xlm);
...
mod tests {
    ...
    fn test_donor_aggregate_on_verify() {
        ...
        // Cumulative totals stay the same.
    ...
    fn test_chain_aggregate_after_verify_and_revoke() {
        ...
        // Verify one, check chain aggregate status shift.
        ...
        // Revoke it and check shift again.
    ...
    fn test_multiple_attestations_across_multiple_chains() {
        ...
        // Check donor aggregate — totals and per-chain.
        let donor_agg = client.get_donor_aggregate(&donor);
        assert_eq!(donor_agg.total_attestations, 4);
        assert_eq!(donor_agg.total_usd, 10_000_000); // 1+2+3+4 million
        assert_eq!(donor_agg.total_xlm, 80_000_000); // 8+16+24+32 million
        assert_eq!(donor_agg.pending, 4);
        assert_eq!(donor_agg.verified, 0);
        assert_eq!(donor_agg.revoked, 0);

        // Check per-chain breakdown in donor aggregate.
        assert_eq!(donor_agg.chains.len(), 3);
        // Use a helper to find chain counts.
    ...
    fn test_aggregate_consistency_after_repeated_operations() {
        ...
        // Verify id1: pending=2, verified=1
        client.verify_attestation(&id1);
        ...
        // Revoke id2 (pending): pending=1, verified=1, revoked=1
        ...
        // Verify id3: pending=0, verified=2, revoked=1
        ...
        // Revoke id1 (verified): verified=1, revoked=2
        client.revoke_attestation(&admin, &id1);
        agg = client.get_donor_aggregate(&donor);
        assert_eq!(agg.pending, 0);
        assert_eq!(agg.verified, 1);
        ...
        // Chain aggregate should also reflect the same breakdown.
