...
function loadJson(filePath) {
  try {
    ...
    console.error(
      `Error loading or parsing JSON file at ${filePath}:`,
      err.message,
    );
  }
  return null;
...
function runTriage() {
  ...
  if (triagedFindings.length > 0) {
    ...
    for (const f of triagedFindings) {
      console.log(
        `[TRIAGED] ${f.alert} (Plugin ID: ${f.pluginId}) on URI: ${f.uri}`,
      );
    }
  }
  ...
  if (activeHighFindings.length > 0) {
    ...
    console.error("\nTo resolve this:");
    console.error(
      "1. Fix the underlying security vulnerability in the codebase (recommended).",
    );
    console.error(
      "2. Or if it is verified as a false positive, add it to zap-false-positives.json.",
    );
    process.exit(1);
  }
