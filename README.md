...
- 🛰 **Production-grade ops** — Helm, ArgoCD, Prometheus, Alertmanager with PagerDuty/Slack routing, monthly restore drills.
...
The setup script installs Node deps for the backend, frontend, mobile, and extension, adds the `wasm32-unknown-unknown` target, and verifies the toolchain.
...
Upgrade mechanics: [`contracts/indigopay-contract/UPGRADE.md`](contracts/indigopay-contract/UPGRADE.md).
...
- Alert rules: 5xx rate, p99 latency, DB pool wait, slow query p99, readiness failing, `BackendDown`, `BackupMissed`, `RestoreDrillFailed`
...
- [**`docs/backend/`**](docs/backend/) — auto-generated TypeDoc site for the backend service layer
...
If you find a vulnerability, **please do not open a public issue.** Use [GitHub Security Advisories](https://github.com/your-org/stellar-indigopay/security/advisories/new) or contact the maintainers privately. See [**`SECURITY.md`**](SECURITY.md) for the response SLA (acknowledgement within 48h, patch within 30d for critical issues).

---

## 🗺 Roadmap

- ✅ v1.0 — Foundation: wallet connect, project browse, donations, leaderboard, Soroban ledger
- ✅ v1.1 — Developer experience: Docker Compose, CI across all layers, unit + e2e tests
- ✅ v1.2 — Verified projects: admin review, on-chain registration
- ✅ v1.3 — Impact NFT badges (Seedling / Tree / Forest / Earth Guardian)
- 🚧 v1.4 — Community features (follow, comments, monthly digests)
- 🚧 v1.5 — Impact dashboard (global map, real-time stream)
- 🚧 v2.0 — USDC alongside XLM via Stellar DEX + fiat conversion
- 🚧 v2.1 — DAO governance: token-weighted voting on project verification
